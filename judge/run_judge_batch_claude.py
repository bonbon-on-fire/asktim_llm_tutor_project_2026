"""Claude batch judge runner for batch_XX_YYY transcript sets.
...
try:
    from .run_judge_claude import JudgeError, JudgeResult, judge_transcript
except ImportError:  # direct script-style import fallback
    from run_judge_claude import JudgeError, JudgeResult, judge_transcript  # type: ignore

_BATCH_DIR = Path(__file__).resolve().parent / "transcript_batches"
_BATCH_OUTPUT_DIR = _REPO_ROOT / "transcripts" / "batch" / "claude"
...
def run_batch_type(
    *,
    ...
    print(
        f"[Claude Batch Judge] Running {len(files)} files for type={normalized} "
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Run Claude judging for all batch files of a given type.")
    parser.add_argument(
        "--batch-type",
        default=BATCH_TYPE,
