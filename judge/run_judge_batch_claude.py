"""Claude-based batch judge for humanities tutor transcripts.
...
from judge.run_judge_claude import (
    JudgeError,
    _coerce_int,
    _create_judge_graph,
    _env_truthy,
...
# ---------------------------------------------------------------------------
_LOG_PATH = _REPO_ROOT / "logs" / "judge_batch_claude_debug.jsonl"
...
def _setup_debug_logger() -> logging.Logger:
    logger = logging.getLogger("judge_batch_claude_debug")
    if logger.handlers:
        return logger
...
def judge_transcript_batch(
    ...
    *,
    ...
    grade_payload["model"] = {"provider": "anthropic", "model": model_name}
