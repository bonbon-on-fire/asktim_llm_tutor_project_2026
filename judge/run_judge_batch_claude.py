"""Claude batch judge wrapper (unified core)."""
...
from judge.run_judge_batch_unified import JudgeError, JudgeResult, judge_transcript_batch_claude
...
judge_transcript_batch = judge_transcript_batch_claude
...
def run_all_batches_of_type(batch_type: int) -> None:
    ...
    print(f"Running Claude batch type {batch_type:02d} ({count} files)")
    failures = 0
    for i in range(1, count + 1):
        batch_file = Path("judge/transcript_batches") / f"{prefix}_{i:03d}.txt"
        out_name = f"{prefix}_{i:03d}__{JUDGE_PROMPT}__{RUBRIC_NAME}__claude.json"
