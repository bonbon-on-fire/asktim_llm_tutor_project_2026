...
echo "[startup] review_ui — read-only conversation review dashboard"
...
if [ -z "${REVIEW_PASSWORD:-}" ]; then
    echo "[error] REVIEW_PASSWORD not set — refusing to start an ungated review tool" >&2
    exit 1
fi
echo "[startup] ✓ REVIEW_PASSWORD is set"
...
if [ -z "${REVIEW_SECRET_KEY:-}" ]; then
    echo "[startup] ⚠ REVIEW_SECRET_KEY not set — using an insecure default; set it for real deploys" >&2
fi
...
# other services. REVIEW_DATABASE_URL wins; else DATABASE_URL.
normalize_pg() {
    case "$1" in
...
if [ -n "${REVIEW_DATABASE_URL:-}" ]; then
    REVIEW_DATABASE_URL="$(normalize_pg "$REVIEW_DATABASE_URL")"
    export REVIEW_DATABASE_URL
    echo "[startup] ✓ REVIEW_DATABASE_URL set: ${REVIEW_DATABASE_URL%@*}@..." # hide creds
elif [ -n "${DATABASE_URL:-}" ]; then
    DATABASE_URL="$(normalize_pg "$DATABASE_URL")"
    export DATABASE_URL
    echo "[startup] ✓ DATABASE_URL set: ${DATABASE_URL%@*}@..." # hide creds
else
    echo "[error] No REVIEW_DATABASE_URL or DATABASE_URL set" >&2
    exit 1
fi
...
exec gunicorn review_ui.run_app:app \
  --bind "0.0.0.0:${PORT}" \
