"""sandbox_ui — production-shape embeddable tutor app.

Loads the project's `.env` from the repo root at package import time so every
entrypoint (`python -m sandbox_ui`, smoke tests, `gunicorn sandbox_ui.run_app:app`)
...
`.env` loading out of `tutor/`, `judge/`, and `sandbox_ui/`.
"""
