...
from review_ui.auth import check_password, clear_auth, mark_authed
from review_ui.services import conversations as svc
...
def index():
    return render_template(
        "index.html",
        title=current_app.config["REVIEW_TITLE"],
        accent=current_app.config["REVIEW_ACCENT"],
    )
...
@review_bp.get("/logout")
def logout():
    clear_auth()
    return redirect(url_for("review.login"))


@review_bp.get("/api/conversations")
...
@review_bp.get("/api/conversation/<conversation_id>")
...
@review_bp.get("/api/image/<int:image_id>")
