"""Single shared-password gate for review_ui.
...
(``REVIEW_PASSWORD``) unlocks the whole tool for the browser session via a
signed Flask session cookie. Not per-user auth — sufficient for a small internal
review tool, and easy to swap for SSO later.

If ``REVIEW_PASSWORD`` is unset (local dev only), the gate is open.
"""
...
_PUBLIC_ENDPOINTS = {"review.login", "review.login_submit", "health", "static"}


def password_required() -> bool:
    """True if a password is configured (i.e. the gate is active)."""
    return bool(current_app.config.get("REVIEW_PASSWORD"))
...
def check_password(candidate: str) -> bool:
    ...
    expected = current_app.config.get("REVIEW_PASSWORD")
