"""review_ui — read-only interface for reviewing real AskTIM conversation data.
...
differentiated by title and color theme. See ``docs/review_ui_plan.md``.
...
entrypoint (``python -m review_ui``, smoke tests, ``gunicorn
review_ui.run_app:app``) picks up ``REVIEW_DATABASE_URL`` and friends without
manual ``$env:`` setup — mirroring ``test_ui``.
"""
