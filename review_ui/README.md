...
- The models map only the columns shared by `main_ui` and `test_ui`, so the same
...
$env:REVIEW_DATABASE_URL = "<sqlite or postgres url>"   # e.g. the prod public URL
$env:REVIEW_PASSWORD     = "some-shared-password"
$env:REVIEW_SECRET_KEY   = "any-random-string"
python -m review_ui                                      # http://127.0.0.1:5003
```

To review the **production** DB from your machine, set `REVIEW_DATABASE_URL` to
...
| -------- | -------- | ----------- |
| `REVIEW_DATABASE_URL` | Yes (prod) | DB to read. Falls back to `DATABASE_URL`, then local SQLite. |
| `REVIEW_PASSWORD` | Yes (deploy) | Shared password for the login gate. Unset ⇒ open (local dev only); the deploy entrypoint refuses to start without it. |
| `REVIEW_SECRET_KEY` | Recommended | Flask session signing key. Has an insecure dev default. |
| `REVIEW_TITLE` | No | Header + tab title. Default `AskTIM · Database Beta`. |
| `REVIEW_ACCENT` | No | Accent color. Default `#8c1a1b` (MIT crimson, = main_ui). |
| `PORT` | No | Default `5003`. |

## Deploy (Railway)

1. New service in the `tutors (UW, humanities)` project, built from
   [`Dockerfile_review`](../Dockerfile_review) (entrypoint
   [`scripts/railway-entrypoint-review.sh`](../scripts/railway-entrypoint-review.sh)).
2. Set variables:
   - `REVIEW_DATABASE_URL = ${{<main Postgres>.DATABASE_URL}}` (reference the
     Postgres that `humanities-main` uses — shares it over the private network).
   - `REVIEW_PASSWORD` and `REVIEW_SECRET_KEY` (secrets).
3. Generate a domain. The entrypoint fails closed if `REVIEW_PASSWORD` is unset,
