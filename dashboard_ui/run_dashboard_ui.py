...
def _list_transcript_rows() -> list[dict]:
    """Build dashboard rows from all *_raw/ transcripts (one row per raw file)."""
    out: list[dict] = []
    for group in _discover_persona_groups():
        for raw_stem in _raw_stems_for_group(group):
            ...
            grade, grade_error = _counterpart_result(
                group=group, provider="claude", raw_stem=raw_stem, raw_data=raw_data
            )
            ...
            out.append(
                {
                    ...
                    "max_score": grade.get("max_score") if grade else None,
...
def api_get_transcript(group: str, version: str):
    """Return full detail for one raw transcript as JSON; version is the raw stem; 404 if not found."""
    ...
    grade, grade_error = _counterpart_result(
        group=group, provider="claude", raw_stem=version, raw_data=raw_data
    )
    ...
    return jsonify(
        {
            ...
            "exchanges_mini": [],
            "resume_from_turn": None,
            "mini_tutor_prompt": None,
            "claude_grade": grade,
            "claude_error": grade_error,
        }
    )
