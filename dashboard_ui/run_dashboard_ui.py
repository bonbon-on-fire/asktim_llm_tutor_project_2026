...
def _discover_persona_groups() -> list[str]:
    """Return persona group names that have a *_claude_tutor_05/ subfolder."""
    ...
    for persona_dir in sorted(p for p in TRANSCRIPTS_DIR.iterdir() if p.is_dir()):
        ...
        tutor05_dir = persona_dir / f"{persona}_claude_tutor_05"
        if tutor05_dir.is_dir():
...
def _tutor05_stems_for_group(group: str) -> list[str]:
    """Return numerically sorted stems from *_claude_tutor_05/ for a persona group."""
    tutor05_dir = TRANSCRIPTS_DIR / group / f"{group}_claude_tutor_05"
    return sorted(_json_stems(tutor05_dir), key=_stem_sort_key)
...
def _list_transcript_rows() -> list[dict]:
    """Build dashboard rows from *_claude_tutor_05/ graded transcripts."""
    out: list[dict] = []
    for group in _discover_persona_groups():
        for stem in _tutor05_stems_for_group(group):
            path = TRANSCRIPTS_DIR / group / f"{group}_claude_tutor_05" / f"{stem}.json"
            ...
            out.append(
                {
                    ...
                    "metadata": meta,
                    "claude_grade": grade,
                    "claude_error": None if grade else "Grade missing from file.",
                    "claude_score": grade["total_score"] if grade else None,
                    "claude_max": grade["max_score"] if grade else None,
                }
            )
    return out
...
def api_get_transcript(group: str, version: str):
    ...
    path = TRANSCRIPTS_DIR / group / f"{group}_claude_tutor_05" / f"{version}.json"
    data = _load_json(path)
    if not data:
    ...
    return jsonify(
        {
            ...
            "metadata": {k: v for k, v in data.items() if k not in ("exchanges", "grade")},
